...
// This is separate from sign.go's withMasterKey to avoid import cycles.
func withMasterKeyForVerify(cfg *config.Config, fn func(*keys.Ed25519Signer) error) error {
...
func VerifySignature(cfg *config.Config, sigFile, dataFile string, statusFd int) error {
	...
	return withMasterKeyForVerify(cfg, func(masterKey *keys.Ed25519Signer) error {
		// Read signature file
